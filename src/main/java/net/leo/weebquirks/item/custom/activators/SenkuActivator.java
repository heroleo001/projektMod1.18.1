...
public class SenkuActivator extends Item implements ICurioItem {
    public SenkuActivator(Properties properties) {
        super(properties);
    }
    ...
    public void onEquip(SlotContext slotContext, ItemStack prevStack, ItemStack stack) {
        ...
        if (player.getPersistentData().getIntArray(WeebQuirks.MOD_ID+ "ability").length != 0) {
            if (!(player.getPersistentData().getIntArray(WeebQuirks.MOD_ID + "ability")[0] == 4)) {
                AbilitySetters.setSenkuAbility(player);
            }
        } else {
            AbilitySetters.setSenkuAbility(player);
        }
