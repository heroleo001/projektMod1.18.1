...
public class ErenActivator extends Item implements ICurioItem {
    public ErenActivator(Properties properties) {
        super(properties);
    }
    ...
    public void onEquip(SlotContext slotContext, ItemStack prevStack, ItemStack stack) {
        ...
        if (player.getPersistentData().getIntArray(WeebQuirks.MOD_ID+ "ability").length != 0) {
            if (!(player.getPersistentData().getIntArray(WeebQuirks.MOD_ID + "ability")[0] == 2)) {
                AbilitySetters.setErenAbility(player);
            }
        } else {
            AbilitySetters.setErenAbility(player);
        }
