...
public class SinraActivator extends Item implements ICurioItem {
    public SinraActivator(Properties properties) {
        super(properties);
    }
    ...
    public void onEquip(SlotContext slotContext, ItemStack prevStack, ItemStack stack) {
        ...
        if (player.getPersistentData().getIntArray(WeebQuirks.MOD_ID+ "ability").length != 0) {
            if (!(player.getPersistentData().getIntArray(WeebQuirks.MOD_ID + "ability")[0] == 3)) {
                AbilitySetters.setSinraAbility(player);
            }
        } else {
            AbilitySetters.setSinraAbility(player);
        }
